...
GitHub認証の追加実装: 接続設定に「GitHub CLIの認証を使う」と「認証状態を再確認」を配置する。CLIの選択中アカウントと、アプリが使用するアカウントを表示する。未ログイン時はターミナルの`gh auth login --hostname github.com --web`を案内する。手動トークン登録は折りたたみ欄から選べる。エラー時も認証欄を残し、再試行・方式変更を可能にする。[実装画面（模擬データ）](screenshots/github-auth.png)
